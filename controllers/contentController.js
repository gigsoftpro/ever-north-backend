...
async function attachMedia(req, row, ...fields) {
  ...
  for (const field of fields) {
    ...
    if (row[idField]) {
      const [rows] = await pool.execute('SELECT * FROM media WHERE id = ?', [row[idField]]);
      row[field] = rows[0] ? { ...rows[0], url: mediaUrl(req, rows[0].path) } : null;
    }
  }
  return row;
}
...
async function updateHero(req, res, next) {
  try {
    const { title, highlighted_word, cta_text, bg_image_id, overlay_image_id } = req.body;
    await pool.execute(
      `UPDATE hero_section SET
       ...
       WHERE id = 1`,
      [title, highlighted_word, cta_text, bg_image_id ?? null, overlay_image_id ?? null]
    );
    const [[row]] = await pool.execute('SELECT * FROM hero_section LIMIT 1');
    const data = await attachMedia(req, row, 'bg_image', 'overlay_image');
...
async function updateAbout(req, res, next) {
  try {
    const {
      about_image_id, mission_badge, mission_text_1, mission_text_2,
    ...
    await pool.execute(
      `UPDATE about_section SET
       ...
       WHERE id = 1`,
      [about_image_id ?? null, mission_badge, mission_text_1, mission_text_2,
       about_badge, about_text_1, about_text_2, about_text_3, services_label]
    );
    const [[row]] = await pool.execute('SELECT * FROM about_section LIMIT 1');
    const data = await attachMedia(req, row, 'about_image');
...
async function getServices(req, res, next) {
  try {
    ...
    const data = await Promise.all(rows.map(r => attachMedia(req, r, 'image')));
    res.json({ success: true, data });
  } catch (err) { next(err); }
}
...
async function createService(req, res, next) {
  try {
    ...
    const [result] = await pool.execute(
      ...
      [title, description || null, image_id || null, style || 'full-text', sort_order || 0]
    );
    const [[row]] = await pool.execute('SELECT * FROM services WHERE id = ?', [result.insertId]);
...
async function updateService(req, res, next) {
  try {
    const { id } = req.params;
    const { title, description, image_id, style, sort_order, is_active } = req.body;
    ...
    await pool.execute(
      `UPDATE services SET
       ...
       WHERE id = ?`,
      [title, description, image_id ?? null, style, sort_order, is_active ?? null, id]
    );
    const [[row]] = await pool.execute('SELECT * FROM services WHERE id = ?', [id]);
    const data = await attachMedia(req, row, 'image');
...
async function deleteService(req, res, next) {
  try {
    const [existing] = await pool.execute('SELECT id FROM services WHERE id = ?', [req.params.id]);
    if (!existing.length) return res.status(404).json({ success: false, message: 'Service not found' });
...
async function getCleaningSection(req, res, next) {
  try {
    const [[meta]] = await pool.execute('SELECT * FROM cleaning_section_meta LIMIT 1');
    const [items] = await pool.execute(
      'SELECT * FROM cleaning_services ORDER BY sort_order ASC'
    );
    const itemsWithMedia = await Promise.all(items.map(r => attachMedia(req, r, 'image')));
...
async function createCleaningItem(req, res, next) {
  try {
    ...
    const [[row]] = await pool.execute('SELECT * FROM cleaning_services WHERE id = ?', [result.insertId]);
    const data = await attachMedia(req, row, 'image');
...
async function updateCleaningItem(req, res, next) {
  try {
    ...
    const [existing] = await pool.execute('SELECT id FROM cleaning_services WHERE id = ?', [id]);
    if (!existing.length) return res.status(404).json({ success: false, message: 'Item not found' });
    await pool.execute(
    ...
    const [[row]] = await pool.execute('SELECT * FROM cleaning_services WHERE id = ?', [id]);
    const data = await attachMedia(req, row, 'image');
...
async function deleteCleaningItem(req, res, next) {
  try {
    const [existing] = await pool.execute('SELECT id FROM cleaning_services WHERE id = ?', [req.params.id]);
    if (!existing.length) return res.status(404).json({ success: false, message: 'Item not found' });
    await pool.execute('DELETE FROM cleaning_services WHERE id = ?', [req.params.id]);
...
async function getMaintenanceSection(req, res, next) {
  try {
    const [[meta]] = await pool.execute('SELECT * FROM maintenance_section LIMIT 1');
    const [items] = await pool.execute('SELECT * FROM maintenance_items ORDER BY sort_order ASC');
...
async function updateMaintenanceMeta(req, res, next) {
  try {
    ...
    const [[meta]] = await pool.execute('SELECT * FROM maintenance_section LIMIT 1');
    res.json({ success: true, message: 'Maintenance meta updated', data: meta });
  } catch (err) { next(err); }
}
...
async function createMaintenanceItem(req, res, next) {
  try {
    ...
    const [[row]] = await pool.execute('SELECT * FROM maintenance_items WHERE id = ?', [result.insertId]);
...
async function updateMaintenanceItem(req, res, next) {
  try {
    ...
    const [existing] = await pool.execute('SELECT id FROM maintenance_items WHERE id = ?', [id]);
    if (!existing.length) return res.status(404).json({ success: false, message: 'Item not found' });
    await pool.execute(
    ...
    const [[row]] = await pool.execute('SELECT * FROM maintenance_items WHERE id = ?', [id]);
...
async function deleteMaintenanceItem(req, res, next) {
  try {
    const [existing] = await pool.execute('SELECT id FROM maintenance_items WHERE id = ?', [req.params.id]);
    if (!existing.length) return res.status(404).json({ success: false, message: 'Item not found' });
    await pool.execute('DELETE FROM maintenance_items WHERE id = ?', [req.params.id]);
    res.json({ success: true, message: 'Item deleted' });
  } catch (err) { next(err); }
}
...
async function getAreasSection(req, res, next) {
  try {
    const [[meta]] = await pool.execute('SELECT * FROM areas_section_meta LIMIT 1');
    const [rows] = await pool.execute('SELECT * FROM areas ORDER BY sort_order ASC');
    const areas = await Promise.all(rows.map(r => attachMedia(req, r, 'image')));
...
async function createArea(req, res, next) {
  try {
    ...
    const [[row]] = await pool.execute('SELECT * FROM areas WHERE id = ?', [result.insertId]);
    const data = await attachMedia(req, row, 'image');
...
async function updateArea(req, res, next) {
  try {
    ...
    if (!existing.length) return res.status(404).json({ success: false, message: 'Area not found' });
    await pool.execute(
      `UPDATE areas SET
        name       = COALESCE(?, name),
        image_id   = COALESCE(?, image_id),
    ...
    const [[row]] = await pool.execute('SELECT * FROM areas WHERE id = ?', [id]);
    const data = await attachMedia(req, row, 'image');
...
async function deleteArea(req, res, next) {
  try {
    const [existing] = await pool.execute('SELECT id FROM areas WHERE id = ?', [req.params.id]);
    if (!existing.length) return res.status(404).json({ success: false, message: 'Area not found' });
...
async function getAllTestimonials(req, res, next) {
  try {
    const [rows] = await pool.execute('SELECT * FROM testimonials ORDER BY sort_order ASC');
    res.json({ success: true, data: rows });
  } catch (err) { next(err); }
}
...
async function createTestimonial(req, res, next) {
  try {
    ...
    if (!customer_name || !quote) {
      return res.status(400).json({ success: false, message: 'customer_name and quote required' });
    }
    const [result] = await pool.execute(
      'INSERT INTO testimonials (customer_name, quote, rating, sort_order) VALUES (?, ?, ?, ?)',
      [customer_name, quote, rating || 5, sort_order || 0]
    );
    const [[row]] = await pool.execute('SELECT * FROM testimonials WHERE id = ?', [result.insertId]);
    res.status(201).json({ success: true, message: 'Testimonial created', data: row });
  } catch (err) { next(err); }
}
...
async function updateTestimonial(req, res, next) {
  try {
    ...
    const [existing] = await pool.execute('SELECT id FROM testimonials WHERE id = ?', [id]);
    if (!existing.length) return res.status(404).json({ success: false, message: 'Testimonial not found' });
    await pool.execute(
    ...
    const [[row]] = await pool.execute('SELECT * FROM testimonials WHERE id = ?', [id]);
...
async function deleteTestimonial(req, res, next) {
  try {
    const [existing] = await pool.execute('SELECT id FROM testimonials WHERE id = ?', [req.params.id]);
    if (!existing.length) return res.status(404).json({ success: false, message: 'Testimonial not found' });
...
async function updateNavLink(req, res, next) {
  try {
    ...
    const [existing] = await pool.execute('SELECT id FROM nav_links WHERE id = ?', [id]);
    if (!existing.length) return res.status(404).json({ success: false, message: 'Nav link not found' });
    await pool.execute(
    ...
    const [[row]] = await pool.execute('SELECT * FROM nav_links WHERE id = ?', [id]);
...
async function updateFooter(req, res, next) {
  try {
    const { logo_id, building_img_id, description, email, phone, copyright_text } = req.body;
    await pool.execute(
      `UPDATE footer_content SET
       ...
       WHERE id = 1`,
      [logo_id ?? null, building_img_id ?? null, description, email, phone, copyright_text]
    );
    const [[row]] = await pool.execute('SELECT * FROM footer_content LIMIT 1');
    const data = await attachMedia(req, row, 'logo', 'building_img');
...
async function getContactSubmissions(req, res, next) {
  try {
    ...
    const [[{ total }]] = await pool.execute('SELECT COUNT(*) AS total FROM contact_submissions');
...
async function getSiteData(req, res, next) {
  try {
    ...
    const [services]  = await pool.execute('SELECT * FROM services WHERE is_active = 1 ORDER BY sort_order');
    const [[cleanMeta]] = await pool.execute('SELECT * FROM cleaning_section_meta LIMIT 1');
    const [cleanItems]  = await pool.execute('SELECT * FROM cleaning_services WHERE is_active = 1 ORDER BY sort_order');
    const [[maintMeta]] = await pool.execute('SELECT * FROM maintenance_section LIMIT 1');
    const [maintItems]  = await pool.execute('SELECT * FROM maintenance_items WHERE is_active = 1 ORDER BY sort_order');
    const [[areasMeta]] = await pool.execute('SELECT * FROM areas_section_meta LIMIT 1');
    const [areas]       = await pool.execute('SELECT * FROM areas WHERE is_active = 1 ORDER BY sort_order');
    const [testimonials]= await pool.execute('SELECT * FROM testimonials WHERE is_active = 1 ORDER BY sort_order');
    const [[headerInfo]]= await pool.execute('SELECT * FROM header_info LIMIT 1');
    const [navLinks]    = await pool.execute('SELECT * FROM nav_links WHERE is_active = 1 ORDER BY sort_order');
    const [[footer]]    = await pool.execute('SELECT * FROM footer_content LIMIT 1');
    ...
    const servicesData= await Promise.all(services.map(r => attachMedia(req, r, 'image')));
    const cleanData   = await Promise.all(cleanItems.map(r => attachMedia(req, r, 'image')));
    const areasData   = await Promise.all(areas.map(r => attachMedia(req, r, 'image')));
...
module.exports = {
  ...
  // Cleaning
  getCleaningSection, updateCleaningMeta,
  createCleaningItem, updateCleaningItem, deleteCleaningItem,
  // Maintenance
  getMaintenanceSection, updateMaintenanceMeta,
  createMaintenanceItem, updateMaintenanceItem, deleteMaintenanceItem,
  // Areas
  getAreasSection, updateAreasMeta, createArea, updateArea, deleteArea,
  // Testimonials
  getTestimonials, getAllTestimonials,
  createTestimonial, updateTestimonial, deleteTestimonial,
  // Header / Nav / Footer
  getHeader, updateHeader,
  getNavLinks, updateNavLink,
  getFooter, updateFooter,
  // Contact
  submitContact, getContactSubmissions, markContactRead, deleteContactSubmission,
  // Full site
  getSiteData,
};
